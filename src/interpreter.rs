...
impl<'a, L: Logger> NanoInterpreter<'a, L> {
    ...
    pub fn with_logger(input: &'a [u8], memory: &'a mut [u8], preserve_memory: bool, logger: Option<&'a L>) -> Self {
        ...
        let mut interpreter = Self {
            ...
            memory,
            control_stack: [ControlFrame { return_pc: 0, is_loop: false, stack_height_at_entry: 0 }; CONTROL_STACK_SIZE],
            control_sp: 0,
            call_stack: [CallFrame { return_pc: 0, prev_control_sp: 0, locals_base: 0 }; MAX_CALL_DEPTH],
            call_sp: 0,
            logger,
        };
                 ...
                 if self.input[data_ptr] == 0x41 {
                     ...
                     if offset_val >= 0 {
                         ...
                         if end <= memory_len && (data_ptr + data_len as usize) <= self.input.len() {
                             self.memory[start..end].copy_from_slice(&self.input[data_ptr..data_ptr + data_len as usize]);
                         }
                     }
    ...
    fn count_imported_functions(&self) -> u32 {
        ...
        if let Ok((mut ptr, _size)) = self.find_section(2) {
             ...
             for _ in 0..vec_len {
                 // module name
                 let (len, l) = Self::read_u32_leb128(&self.input[ptr..]); ptr += l;
                 ptr += len as usize;
                 // field name
                 let (len, l) = Self::read_u32_leb128(&self.input[ptr..]); ptr += l;
                 ptr += len as usize;
                 ...
                 // kind
                 let kind = self.input[ptr]; ptr += 1;
                 if kind == 0 { // Function import
                     let (_type_idx, l) = Self::read_u32_leb128(&self.input[ptr..]); ptr += l;
                     count += 1;
                 } else if kind == 1 { // Table
                     // Table type: element_type(1) + limits
                     ptr += 1; 
                     let flags = self.input[ptr]; ptr += 1;
                     let (_min, l) = Self::read_u32_leb128(&self.input[ptr..]); ptr += l;
                     if flags & 1 != 0 {
                         let (_max, l) = Self::read_u32_leb128(&self.input[ptr..]); ptr += l;
                     }
    ...
    fn find_matching_end(&self, start_pc: usize, max_pc: usize) -> Result<usize, &'static str> {
        ...
        while pc < max_pc && depth > 0 {
            ...
            match opcode {
                ...
                0x20..=0x24 => { // local.get, local.set, local.tee, global.get, global.set
                    let (_, len) = Self::read_u32_leb128(&self.input[pc..]);
                    pc += len;
                }
                0x28..=0x3E => { // memory loads/stores
                ...
                0xFC => { // Extended opcodes prefix
                    if pc < self.input.len() {
                        let extended_opcode = self.input[pc];
                        pc += 1;
                        match extended_opcode {
                            0x01 | 0x0C => { // data.drop, elem.drop
                                let (_, len) = Self::read_u32_leb128(&self.input[pc..]);
                                pc += len;
                            }
                            0x02 | 0x03 => { // memory.copy, memory.fill
                                let (_, len) = Self::read_u32_leb128(&self.input[pc..]);
                                pc += len;
                            }
                            0x0B => { // table.init
    ...
    fn execute_loop(&mut self, end_pc: usize) -> Result<i32, &'static str> {
        ...
        while self.pc < end_pc {
            ...
            match opcode {
                0x00 => { return Err("Unreachable executed"); }
                0x01 => { } // nop
                0x02 => { // block
                ...
                0x4A => { // i32.gt_u (unsigned greater-than)
                     let v2 = self.pop()? as u32;
                     let v1 = self.pop()? as u32;
                     self.push(if v1 > v2 { 1 } else { 0 })?;
                }
                0x4B => { // i32.gt_s (signed greater-than)
                     let v2 = self.pop()? as i32;
                     let v1 = self.pop()? as i32;
                     self.push(if v1 > v2 { 1 } else { 0 })?;
                }
                ...
                0x28 => { // i32.load
                    let (_align, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let (offset, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                ...
                0x2C => { // i32.load8_u (load unsigned 8-bit)
                    let (_align, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let (offset, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let base = self.pop()? as usize;
                ...
                0x2D => { // i32.load8_s (load signed 8-bit)
                    let (_align, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let (offset, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let base = self.pop()? as usize;
                    let addr = base + offset as usize;
                ...
                0x2E => { // i32.load16_u (load unsigned 16-bit)
                    let (_align, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let (offset, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let base = self.pop()? as usize;
                    let addr = base + offset as usize;
                    if addr + 2 <= memory_len {
                        let val = u16::from_le_bytes([self.memory[addr], self.memory[addr+1]]) as u32;
                        self.push(val as u64)?;
                    } else {
                        return Err("Memory access out of bounds");
                    }
                }
                0x2F => { // i32.load16_s (load signed 16-bit)
                    let (_align, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let (offset, len) = Self::read_u32_leb128(&self.input[self.pc..]); self.pc += len;
                    let base = self.pop()? as usize;
                    let addr = base + offset as usize;
                    if addr + 2 <= memory_len {
                        let val = i16::from_le_bytes([self.memory[addr], self.memory[addr+1]]) as i32;
                        self.push(val as u64)?;
                    } else {
                        return Err("Memory access out of bounds");
                    }
                ...
                // f64 Conversions
                0xAA => { // i32.trunc_f64_s (signed truncation)
                    let val = self.pop_f64()?;
                    // WebAssembly truncation: rounds toward zero
                    let result = libm::trunc(val) as i32;
                    self.push(result as u64)?;
                }
                0xAB => { // i32.trunc_f64_u (unsigned truncation)
                    let val = self.pop_f64()?;
                ...
                0xB7 => { // f64.convert_i32_s (signed conversion)
                    let val = self.pop()? as i32;
                    self.push_f64(val as f64)?;
                }
                0xB8 => { // f64.convert_i32_u (unsigned conversion)
                    let val = self.pop()? as u32;
                    self.push_f64(val as f64)?;
                }
